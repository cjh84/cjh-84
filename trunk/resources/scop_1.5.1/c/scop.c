...
int main(int argc, char **argv)
{
	...
	sock = scop_open(remote_hostname, "CLI", 0);
	if(sock == -1)
	{
	...
	if(mode == MODE_LIST)
	{
		...
		for(i = 0; i < count; i++)
		{
			printf("Client connection <%s> listening to <%s>, "
	...
	else if(mode == MODE_MSG)
	{
		for(i = 0; i < num_clients; i++)
		{
			...
			scop_send_message(sock, client, message, 0);
		}
	}
	...
	else if(mode == MODE_XMLRPC)
	{
		...
		if(num_args > 0)
		{
			...
			if(!strcmp(method, "-"))
				w = scop_rpc_x(sock, client, v, NULL);
			else
				w = scop_rpc_x(sock, client, v, method);
			free(v);
	...
	else if(mode == MODE_XMLSEND)
	{
		...
		{
			// No args - just send the method name (or "-") as plain text:
			scop_send_message(sock, client, method, 0);
		}
		else if(num_args == 1)
		{
			// Single argument, pack it but don't need a list:
		...
		{
			if(!strcmp(method, "-"))
				scop_send_struct(sock, client, v, NULL);
			else
