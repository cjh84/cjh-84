...
int main(int argc, char* argv[])
{
	...
	// pHostInfoEntry stores all retrieved info about the Vicon server
	struct hostent*		pHostInfoEntry;
	// This info is then used to create the appropriate Endpoint (socket spec for Vicon server)
	...
	// The cast is necessary because IPv4 and IPv6 have their own socket specs in sockaddr_in and sockaddr_in6
	...
	try
	{
		...
		* ((long int *) pBuff) = ClientCodes::ERequest;
		...
		// receive packet kind (Info, Data etc.)
		if(!recieve(SocketHandle, packet))
			throw std::string("Error Recieving");

		// receive packet type (Request/Reply)
		...
		// read channel information and store it in string vector info
		for(iInfo = info.begin(); iInfo != info.end(); iInfo++)
		{
			...
			// receive number of letters in string
			if(!recieve(SocketHandle, s)) 
				throw std::string();
			
			// receive string of length s in buffer c
		...
		for(iInfo = info.begin(); iInfo != info.end(); iInfo++)
		{
			...
			// set up the correct MarkerChannel or BopdyChannel ints (X, Y, Z, O, TX, TY etc ...)
		...
		for(i = 0; i < 1000; i++)
		// i guess for testing we just receive 1000 packets and then exit ...
		{
			pBuff = buff;
			...
			* ((long int *) pBuff) = ClientCodes::ERequest;
			...
			// read length of data packet (nmber of channels sent)
			...
			//	Get the data.
			// i.e. read double values for all the channels
			...
			{
				// assign MarkerData elements the correct elements in the data vector
			...
			//=================================================================
			// most of the code in here deals with the correct rotation transfomation
			// in this case exponential map --> Euler (for Graphics rendering)
